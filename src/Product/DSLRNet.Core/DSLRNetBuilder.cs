    ...
    IOperationProgressTracker progressTracker)
{
    ...
    public async Task UpdateMessages(List<ParamEdit> paramEdits)
    {
        ...
        {
            if (fileSourceHandler.TryGetFile(Path.Combine("msg", settings.MessageLocale, Path.GetFileName(fileName)), out string existingPath))
            {
            ...
            {
                throw new Exception($"Could not find message file {Path.GetFileName(fileName)} for locale {settings.MessageLocale}");
            }
        }

        Directory.CreateDirectory(Path.Combine(this.settings.DeployPath, "msg", settings.MessageLocale));
